...
The app uses [Create React App](https://github.com/facebookincubator/create-react-app) as a starting point.
...
While you can still use `require()` and `module.exports`, it's better to use [`import` and `export`](http://exploringjs.com/es6/ch_modules.html) instead.
...
If you’d like to test components in isolation from the child components they render, it is recommended to use [`shallow()` rendering API](http://airbnb.io/enzyme/docs/api/shallow.html) from [Enzyme](http://airbnb.io/enzyme/). You can write a smoke test with it too:
...
If you are used to other libraries, such as [Chai](http://chaijs.com/) and [Sinon](http://sinonjs.org/), or if you have existing code using them that you’d like to port over, you can import them normally like this:
